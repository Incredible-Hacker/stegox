...
from stegox.modules.metadata import list_fields, list_strategies

app = typer.Typer(help="Metadata hide, extract, detect, and analyze.")
...
def hide_cmd(
    ...
    field: str = typer.Option("UserComment", "--field", help="Metadata field name."),
    json_output: bool = typer.Option(False, "--json"),
) -> None:
    try:
        data = payload.read_bytes()
        result = engine_hide(cover, data, strategy=field, stego=out, media_type="metadata")
        if json_output:
            _emit({"stego": str(result.stego_path), "field": field, "payload_bytes": result.payload_bytes})
        else:
...
def extract_cmd(
    ...
    out: Path = typer.Option(..., "--out"),
    field: str = typer.Option("UserComment", "--field"),
    json_output: bool = typer.Option(False, "--json"),
) -> None:
    try:
        data = engine_extract(target, strategy=field, out=out, media_type="metadata")
        if json_output:
...
def audit_cmd(
    ...
    from stegox.modules.metadata import audit_metadata
    report = audit_metadata(target)
    if json_output:
        _emit(report)
    else:
...
def list_fields_cmd() -> None:
    typer.echo(json.dumps(list_fields(), indent=2))
