...
app = typer.Typer(help="Text hide, extract, detect, and analyze.")
...
def hide_cmd(
    ...
    strategy: str = typer.Option("zero-width", "--strategy"),
    json_output: bool = typer.Option(False, "--json"),
) -> None:
    try:
        data = payload.read_bytes()
        result = engine_hide(cover, data, strategy=strategy, stego=out, media_type="text")
        if json_output:
...
def extract_cmd(
    ...
    try:
        data = engine_extract(stego, strategy=strategy, out=out, media_type="text")
        if json_output:
