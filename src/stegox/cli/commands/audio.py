...
app = typer.Typer(help="Audio hide, extract, detect, and analyze.")
...
def hide_cmd(
    ...
    strategy: str = typer.Option("pcm-lsb", "--strategy"),
    ...
    try:
        ...
        result = engine_hide(cover, data, strategy=strategy, password=pw, stego=out, media_type="audio")
        if json_output:
            _emit({"stego": str(result.stego_path), "strategy": result.strategy, "payload_bytes": result.payload_bytes})
        else:
...
def extract_cmd(
    ...
    try:
        pw = _read_password(password)
        data = engine_extract(stego, strategy=strategy, password=pw, payload_bytes=payload_bytes, out=out, media_type="audio")
        if json_output:
